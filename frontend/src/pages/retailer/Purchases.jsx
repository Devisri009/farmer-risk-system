...
const Purchases = () => {
    ...
    useEffect(() => {
        const fetchPurchases = async () => {
            try {
                const response = await apiClient.get('/retailer/purchases');
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            {loading ? (
                ...
                <div className="p-12 text-center text-gray-500">
                    ...
                    <Link to="/retailer/marketplace" className="inline-block bg-primary-green text-white px-6 py-2 rounded-lg font-semibold hover:bg-green-700 transition">
                        Browse Marketplace
                    </Link>
                </div>
            ) : (
                        ...
                        {purchases.map((purchase) => (
                            <tr key={purchase.id} className="hover:bg-gray-50/80 transition-colors group">
                                ...
                                <td className="px-6 py-4">
                                    <div className="font-bold text-gray-900">{purchase.crop_name} • {purchase.quantity}</div>
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end items-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                                        <button 
                                            ...
                                            className="text-xs text-blue-700 hover:text-blue-900 font-mono px-2.5 py-1.5 bg-blue-50 hover:bg-blue-100 rounded-lg border border-blue-200 font-bold transition-all flex items-center gap-1"
                                        ...
                                        <button 
                                            ...
                                            className="p-2 text-primary-green hover:bg-green-50 rounded-lg transition-colors border border-transparent hover:border-green-100" 
                    ...
                    <div className="bg-white rounded-2xl shadow-xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                            <h3 className="text-xl font-bold text-gray-900">
                                {modalType === 'resell' ? 'Resell Crop Batch' : modalType === 'audit' ? 'Blockchain Provenance Audit' : 'Traceability QR Code'}
                            </h3>
                            <button onClick={closeModal} className="text-gray-400 hover:text-gray-600 p-1 bg-white hover:bg-gray-100 rounded-full transition-colors border border-gray-200">
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="p-6">
                            {modalType === 'resell' && selectedPurchase && (
                                <div className="space-y-4">
                                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                        <p className="text-sm text-gray-500 mb-1">Purchased Item</p>
                                        <p className="font-bold text-gray-900">{selectedPurchase.crop_name} • {selectedPurchase.quantity}</p>
                                    ...
                                    <button className="w-full py-3 bg-primary-green text-white font-bold rounded-xl mt-4 hover:bg-green-700 transition shadow-lg shadow-green-900/20 flex justify-center items-center gap-2">
                            ...
                            {modalType === 'audit' && selectedPurchase && (
                                <div className="space-y-4">
                                    <div className="p-4 bg-gradient-to-br from-slate-900 to-indigo-950 text-white rounded-xl border border-blue-900/60 shadow-inner">
                                        ...
                                        <p className="text-sm font-bold">{selectedPurchase.crop_name} • {selectedPurchase.quantity}</p>
                                        <p className="text-xs text-blue-200/70 mt-0.5">Farmer: {selectedPurchase.farmer_name}</p>
                                    </div>
                                    ...
                                    <div className="space-y-2 text-xs">
                                        ...
                                        <div className="flex justify-between py-1.5 border-b border-gray-100">
                                            <span className="text-gray-500 font-semibold">Date Settled</span>
                                    ...
                                    <div className="flex gap-2 pt-2">
                                        ...
                                        <button 
                                            ...
                                            className="px-4 py-2.5 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition text-xs flex items-center justify-center gap-1 border border-gray-200"
                                        >
                                            <Copy size={13} />
                                            <span>Copy</span>
                                        </button>
                            ...
                            {modalType === 'qr' && selectedPurchase && (
                                <div className="flex flex-col items-center text-center space-y-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-gray-900 text-lg">{selectedPurchase.crop_name}</h4>
                                        <p className="text-sm text-gray-500 font-mono mt-1">Batch ID: {selectedPurchase.id}</p>
                                    </div>
                                    <p className="text-sm text-gray-600 bg-blue-50 text-blue-800 p-3 rounded-xl border border-blue-100">
                                        Print or display this QR code to allow consumers to track the crop's journey on the blockchain.
                                    </p>
                                    <div className="flex gap-3 w-full mt-4">
                                        ...
                                        <button className="flex-1 py-2.5 bg-primary-green text-white font-bold rounded-xl hover:bg-green-700 transition shadow-lg shadow-green-900/20" onClick={() => window.alert("Downloading QR Code...")}>
                                            Download
                                        </button>
                                    </div>
                                </div>
